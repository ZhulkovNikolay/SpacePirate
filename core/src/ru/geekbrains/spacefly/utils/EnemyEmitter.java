package ru.geekbrains.spacefly.utils;
...
//создает врагов
//каждый корабль создается из пула объектов.
//при взрыве уходит в режим ожидания
public class EnemyEmitter {
    ...
    public EnemyEmitter(TextureAtlas atlas, Rect worldBounds, EnemyPool enemyPool) {
        ...
        //достаем текстуру врага
        TextureRegion textureRegion0 = atlas.findRegion("enemy0");
        //2 состояния для каждого корабля, 1 строчка
    ...
    public void generateEnemies(float delta, int frags) {
        level = frags / 10 + 1; //сколько убить для перехода на след уровень
        generateTimer += delta;
        if (generateTimer >= generateInterval) {
            generateTimer = 0f; //обнуляем
            Enemy enemy = enemyPool.obtain(); //достаем объект из пула
            float type = (float) Math.random(); //вероятность появления
            if (type < 0.7f) {
                enemy.set(
                        enemySmallRegion,
                        enemySmallV,
                        ...
                        ENEMY_SMALL_BULLET_DAMAGE * level, // с ростом уровня сила врага увеличичвается
            ...
            //чтобы появлялся сверху и поместился полностью
